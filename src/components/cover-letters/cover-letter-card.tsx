...
export function CoverLetterCard({
  ...
  const status = STATUS_CONFIG[coverLetter.status] ?? STATUS_CONFIG.DRAFT;
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-2 flex flex-wrap items-center gap-3 text-xs text-muted-foreground">
            {coverLetter.jobTitle && coverLetter.jobCompany && (
              <span className="flex items-center gap-1">
                <Briefcase className="h-3 w-3" />
                <span className="truncate max-w-[180px]">
                  {coverLetter.jobTitle} at {coverLetter.jobCompany}
                </span>
              </span>
            )}
            {!coverLetter.hasContent && (
                      ...
                      {ALL_STATUSES.map((s) => {
                        ...
                        return (
                          <button
                            ...
                            onClick={(e) => {
                              e.stopPropagation();
                              onStatusChange(coverLetter.id, s);
                              setMenuOpen(false);
                              setStatusMenuOpen(false);
                            }}
                            className={`flex w-full items-center gap-2 px-3 py-1.5 text-sm hover:bg-muted ${
                              coverLetter.status === s
